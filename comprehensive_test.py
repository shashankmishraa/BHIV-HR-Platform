...
def test_comprehensive_endpoints():
    ...
    for method, url in endpoints:
        try:
            if method == "GET":
                ...
                test_data = {"test": True}
                response = httpx.post(
                    url, headers=headers, json=test_data, timeout=10.0
                )
            elif method == "PUT":
                test_data = {"test": True}
    ...
    if results["non_functional"]:
        ...
        for endpoint in results["non_functional"]:
            print(
                f"  {endpoint['method']} {endpoint['url'].split('/')[-1]} - {endpoint['status']} ({endpoint.get('issue', 'Unknown issue')})"
            )
    ...
    if results["errors"]:
        ...
        for endpoint in results["errors"]:
            print(
                f"  {endpoint['method']} {endpoint['url'].split('/')[-1]} - {endpoint['error']}"
            )
