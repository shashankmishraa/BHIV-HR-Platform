...
class SecurityAuditor:
    ...
    def generate_remediation_plan(self) -> List[str]:
        ...
        if any(
            item["risk"] == "HIGH" for item in self.audit_results["insecure_configs"]
        ):
            remediation_steps.extend(
                [
    ...
    def run_comprehensive_audit(self) -> Dict[str, Any]:
        ...
        audit_summary = {
            ...
            "security_score": max(
                0, 100 - (high_risk_issues * 25) - (total_issues * 5)
            ),
        }
...
def main():
    ...
    if summary["insecure_configs"] > 0:
        ...
        for config in results["details"]["insecure_configs"][:5]:  # Show first 5
            print(
                f"  - {config.get('file', config.get('variable', 'Unknown'))}: {config['issue']}"
            )
