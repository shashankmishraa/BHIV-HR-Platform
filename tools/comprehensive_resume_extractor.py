...
class ComprehensiveResumeExtractor:
    ...
    def _analyze_name(self, text, filename):
        ...
        for line in lines:
            ...
            if 2 <= len(words) <= 4 and len(line) < 50:
                if all(
                    len(word) > 1 and word.replace(".", "").isalpha() for word in words
                ):
    ...
    def _analyze_education(self, text):
        ...
        if any(
            ...
            for pattern in ["phd", "ph.d", "doctorate", "doctoral degree"]
        ):
            return "PhD"
        ...
        # Diploma patterns
            ...
            pattern in text_lower
            for pattern in ["diploma", "polytechnic", "associate degree"]
        ):
            return "Diploma"
        ...
        # 12th patterns
            ...
            pattern in text_lower
            for pattern in ["12th", "higher secondary", "intermediate", "hsc", "+2"]
        ):
            return "12th"
    ...
    def process_all_resumes(self):
        ...
        for i, file_path in enumerate(resume_files, 1):
            ...
            print(
                f"  Skills: {candidate_data['skills'][:60]}{'...' if len(candidate_data['skills']) > 60 else ''}"
            )
        ...
        if candidates:
            ...
            print(
                f"Emails: {df['email'].astype(bool).sum()} ({df['email'].astype(bool).sum()/len(df)*100:.0f}%)"
            )
            print(
                f"Phones: {df['phone'].astype(bool).sum()} ({df['phone'].astype(bool).sum()/len(df)*100:.0f}%)"
            )
            print(
                f"Locations: {df['location'].astype(bool).sum()} ({df['location'].astype(bool).sum()/len(df)*100:.0f}%)"
            )
            print(
                f"Designations: {df['designation'].astype(bool).sum()} ({df['designation'].astype(bool).sum()/len(df)*100:.0f}%)"
            )
...
def main():
    ...
    if result is not None:
        ...
        try:
            ...
            print(
                "Both HR Portal (8501) and Client Portal (8502) now have real-time data"
            )
        except Exception as e:
