...
async def start_workflow(workflow_id: str, background_tasks: BackgroundTasks):
    ...
    if workflow["status"] != WorkflowStatus.PENDING:
        raise HTTPException(
            status_code=400, detail="Workflow already started or completed"
        )
...
async def get_workflow_analytics():
    ...
    return {
        ...
        "failure_rate": (
            (failed_workflows / total_workflows * 100) if total_workflows > 0 else 0
        ),
...
async def execute_pipeline_template(
    ...
    if background_tasks:
        background_tasks.add_task(
            execute_pipeline, execution_id, template_id, parameters or {}
        )
...
async def delete_workflow(workflow_id: str):
    ...
    return {"workflow_id": workflow_id, "deleted": True, "status": "success"}
...
async def get_workflow_status(workflow_id: str):
    ...
    return {
        ...
        "current_step": (
            workflow["steps"][completed_steps]["name"]
...
async def pause_workflow(workflow_id: str):
    ...
    return {"workflow_id": workflow_id, "paused": True, "status": "paused"}
...
async def resume_workflow(workflow_id: str, background_tasks: BackgroundTasks):
    ...
    return {"workflow_id": workflow_id, "resumed": True, "status": "running"}
    ...
    templates = [
        {
            ...
            "description": "Standard candidate onboarding flow",
            "type": "candidate_onboarding",
        },
        {
            "id": 2,
            "name": "Job Posting",
            "description": "Job posting and approval workflow",
            "type": "job_posting",
        },
        {
            "id": 3,
            "name": "Interview Process",
            "description": "Complete interview management workflow",
...
async def create_workflow_template(
    name: str, description: str, workflow_type: WorkflowType
):
...
async def bulk_trigger_workflows(
    workflow_ids: List[str], background_tasks: BackgroundTasks
):
