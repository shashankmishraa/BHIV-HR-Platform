            ...
            return {
                ...
                "details": {
                    ...
                    "database_size_mb": round(
                        db_stats["db_size_bytes"] / (1024 * 1024), 2
                    ),
            ...
            return {
                ...
                "details": {
                    ...
                    "memory_usage_percent": (
                        round((memory_usage / max_memory * 100), 2)
                        if max_memory > 0
                        else 0
                    ),
...
class HealthCheckManager:
    ...
    async def get_simple_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "checks_passed": len(
                [c for c in system_health.checks if c.status == HealthStatus.HEALTHY]
            ),
...
def create_health_manager(config: Dict[str, Any]) -> HealthCheckManager:
    ...
    for service in config.get("dependent_services", []):
        manager.add_http_service_check(
            service["url"], service["name"], service.get("expected_status", 200)
        )
