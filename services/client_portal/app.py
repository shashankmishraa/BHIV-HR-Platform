...
def main():
    ...
    st.set_page_config(
        page_title="BHIV Client Portal", page_icon=page_icon, layout="wide"
    )
    ...
    try:
        jobs_response = requests.get(
            f"{API_BASE_URL}/v1/jobs", headers=headers, timeout=5
        )
        if jobs_response.status_code == 200:
            ...
            client_jobs = [
                j
                for j in jobs
                if str(j.get("client_id", 0))
                == str(hash(st.session_state.get("client_id", "TECH001")) % 1000)
            ]
    ...
    with tab1:
        ...
        with st.form("client_login"):
            ...
            password = st.text_input(
                "Password", type="password", placeholder="Enter your secure password"
            )
                ...
                if client_id and password:
                    with st.spinner("Authenticating..."):
                        ...
                        if success:
                            ...
                            st.error(
                                f"❌ {result.get('error', 'Authentication failed')}"
                            )
                else:
    ...
    with tab2:
        ...
        with st.form("client_register"):
            new_client_id = st.text_input(
                "Choose Client ID", placeholder="e.g., MYCOMPANY01"
            )
            company_name = st.text_input(
                "Company Name", placeholder="Your Company Ltd."
            )
            contact_email = st.text_input(
                "Contact Email", placeholder="admin@yourcompany.com"
            )
            new_password = st.text_input(
                "Create Password", type="password", help="Minimum 8 characters"
            )
            ...
            if st.form_submit_button(
                "📝 Secure Registration", use_container_width=True
            ):
                if all(
                    [
                        new_client_id,
                    ...
                    with st.spinner("Creating secure account..."):
                        ...
                        if success:
                            st.success(
                                "✅ Registration successful! You can now login securely."
                            )
                            st.balloons()
                        else:
                            st.error(f"❌ {message}")
...
def show_job_posting():
    ...
    st.info(
        f"🏢 Posting as: {st.session_state.get('client_name', 'Unknown')} (ID: {st.session_state.get('client_id', 'N/A')})"
    )
    ...
    with st.form("job_posting"):
        ...
        with col1:
            ...
            department = st.selectbox(
                "Department",
                ["Engineering", "Marketing", "Sales", "HR", "Operations", "Finance"],
            )
        ...
        if response.status_code == 200:
            ...
            if jobs:
                ...
                if unique_jobs:
                    ...
                    job_options = {
                        f"{job.get('title')} (ID: {job_id})": job_id
                    ...
                    if selected_job:
                        ...
                        try:
                            ...
                            agent_url = (
                                default_agent_url
                                if "default_agent_url" in locals()
                                else "http://agent:9000"
                            )
                            ...
                            if agent_response.status_code == 200:
                                    ...
                                    candidates.append(
                                        {
                                            ...
                                            "values_alignment": min(
                                                5.0, candidate.get("score", 0) / 20
                                            ),
                                ...
                                if candidates:
                                    st.success(
                                        f"Found {len(candidates)} AI-matched candidates (Dynamic Matching)"
                                    )
                                    st.info(
                                        f"Job: {job_details.get('title')} | Algorithm: {agent_data.get('algorithm_version', 'Dynamic AI')}"
                                    )
                                    ...
                                    for i, candidate in enumerate(candidates[:10]):
                                        if isinstance(
                                            ...
                                            with st.expander(
                                                f"Candidate: {candidate.get('name')} (AI Score: {ai_score}/100)"
                                            ):
                                                ...
                                                with col2:
                                                    st.write(
                                                        f"**Experience:** {candidate.get('experience_match', 'N/A')}"
                                                    )
                                                    st.write(
                                                    ...
                                                    if isinstance(skills_match, str):
                                                        ...
                                                        st.write(
                                                            f"**Skills Match:** {skills_match:.1f}%"
                                                        )

                                                with col3:
                                                    st.write(
                                                        f"**Values Score:** {candidate.get('values_alignment', 0):.1f}/5"
                                                    )
                                ...
                                st.error(
                                    f"AI agent failed: {agent_response.status_code}"
                                )
                                ...
                                try:
                                        ...
                                        if candidates:
                                            ...
                                            for i, candidate in enumerate(
                                                ...
                                                if isinstance(
                                                    ...
                                                    with st.expander(
                                                        f"Candidate: {candidate.get('name')} (Score: {candidate.get('score', 0)})"
                                                    ):
                                                        st.write(
                                                            f"Email: {candidate.get('email', 'N/A')}"
                                                        )
                                    ...
                                    st.error(
                                        f"Gateway fallback failed: {str(gateway_error)}"
                                    )
                        except Exception as e:
            ...
            if jobs:
                ...
                if job_titles:
                    ...
                    job_titles_sorted = sorted(
                        job_titles, key=lambda x: int(x.split("ID: ")[1].split(")")[0])
                    )
                    selected_job = st.selectbox(
                        "Select Job for AI Matching", job_titles_sorted
                    )
                        ...
                        with st.spinner("🤖 AI is dynamically analyzing candidates..."):
                            try:
                                ...
                                if response.status_code == 200:
                                    ...
                                    if matches:
                                        st.success(
                                            f"✅ Found {len(matches)} dynamically matched candidates"
                                        )
                                        st.info(
                                            f"📊 Algorithm: {data.get('algorithm_version', 'Dynamic AI')} | Processing: {data.get('processing_time', 0):.3f}s"
                                        )

                                        # Display dynamic matches in clean format
                                        for i, match in enumerate(matches, 1):
                                            if isinstance(match, dict) and match.get(
                                                "name"
                                            ):
                                                ...
                                                with st.container():
                                                    st.markdown(
                                                        f"### {score_color} #{i} - {match.get('name')}"
                                                    )
                                                    ...
                                                    with col1:
                                                        st.write(
                                                            f"**Email:** {match.get('email', 'N/A')}"
                                                        )
                                                        st.write(
                                                            f"**Phone:** {match.get('phone', 'N/A')}"
                                                        )

                                                    with col2:
                                                        st.write(
                                                            f"**Experience:** {match.get('experience_match', 'N/A')}"
                                                        )
                                                        ...
                                                        if (
                                                            ...
                                                            st.write(
                                                                f"**Skills Match:** {', '.join(skills_match[:3])}"
                                                            )
                                                        elif isinstance(
                                                            skills_match, str
                                                        ):
                                        ...
                                        st.info(
                                            "💡 Ensure candidates are uploaded and try again"
                                        )
                                else:
                                    st.error(
                                        f"❌ AI agent failed: {response.status_code}"
                                    )
                                    st.info("Attempting fallback matching...")
                            except Exception as e:
                                st.error(f"❌ Dynamic matching error: {str(e)}")
                                st.info(
                                    "AI agent may be unavailable - check system status"
                                )
                else:
...
def show_reports():
    ...
    try:
        jobs_response = requests.get(
            f"{API_BASE_URL}/v1/jobs", headers=headers, timeout=10
        )
        candidates_response = requests.get(
            f"{API_BASE_URL}/v1/candidates/search", headers=headers, timeout=10
        )
        ...
        if candidates_response.status_code == 200:
            ...
            candidates = (
                ...
                else (candidates_data if isinstance(candidates_data, list) else [])
            )
            unique_candidates = {}
    ...
    with col1:
        st.metric(
            "Active Jobs", str(total_jobs), delta=f"+{max(0, total_jobs-1)} recent"
        )
    with col2:
        st.metric(
            "Total Applications",
            str(total_applications),
            delta=f"+{max(0, total_applications//10)} this week",
        )
    with col3:
        st.metric(
            "Interviews Scheduled", str(interviews_scheduled), delta="+0 this week"
        )
    with col4:
    ...
    st.info(
        f"📊 Based on {total_applications} candidates and {total_jobs} active jobs from database"
    )
    ...
    with col1:
        ...
        st.write(
            f"• AI Screened: {total_applications if total_applications > 0 else 0}"
        )
    ...
    with col2:
        ...
        if total_applications > 0:
            ...
            st.write(
                f"• Reviewed → Interview: {int(interviews_scheduled/total_applications*100) if total_applications > 0 else 0}%"
            )
            st.write(
                f"• Interview → Offer: {int(offers_made/interviews_scheduled*100) if interviews_scheduled > 0 else 0}%"
            )
