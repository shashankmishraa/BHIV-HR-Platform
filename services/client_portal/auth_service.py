...
class ClientAuthService:
    ...
    def _connect_database(self, db_configs):
        ...
        for i, db_url in enumerate(db_configs):
            try:
                logger.info(
                    f"Attempting database connection {i+1}/{len(db_configs)}..."
                )
    ...
    def _create_default_clients(self, connection):
        ...
        for client in default_clients:
            try:
                ...
                logger.warning(
                    f"Could not create default client {client['client_id']}: {e}"
                )
    ...
    def register_client(
        self, client_id: str, company_name: str, email: str, password: str
    ) -> Dict[str, Any]:
        """Register new client with validation"""
        if not self.engine:
            return {
                "success": False,
                "error": "Registration unavailable - database offline",
            }
    ...
    def authenticate_client(self, client_id: str, password: str) -> Dict[str, Any]:
        ...
        try:
            with self.engine.connect() as connection:
                ...
                # Check if account is locked
                if (
                    client_data[4] and client_data[4] > datetime.utcnow()
                ):  # locked_until
