...
def show_batch_upload():
    ...
    upload_method = st.radio(
        "Upload Method", ["Individual Files", "Zip Archive", "Folder Scan"]
    )
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if resume_files:
                ...
                if st.button(
                    "Extract Data and Upload to Database", use_container_width=True
                ):
                    trigger_resume_processing()
            else:
                ...
                if data_file.exists():
                    ...
                    if candidates_data:
                        ...
                        if response.status_code == 200:
                            ...
                            st.success(
                                f"✅ Successfully uploaded {result_data.get('candidates_inserted', 0)} candidates to database!"
                            )
                            st.info(
                                "📊 Candidates are now available for AI matching and search"
                            )
                        else:
                    ...
                    st.warning(
                        "⚠️ No extracted data file found. Please check resume extraction process."
                    )
            else:
