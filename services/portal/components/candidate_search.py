...
def show_candidate_search(API_BASE, headers, SECURITY_ENABLED, sql_guard=None):
    ...
    st.write(
        "Search and filter candidates using AI-powered semantic search and advanced filters"
    )
    ...
    with col2:
        job_filter = st.selectbox(
            "Filter by Job",
            ["All Jobs", "Job ID 1 - Software Engineer", "Job ID 2 - AI/ML Intern"],
        )
    ...
    with filter_col2:
        education_filter = st.multiselect(
            "Education Level", ["Bachelors", "Masters", "PhD", "Diploma"], default=[]
        )
        location_filter = st.multiselect(
            "Location",
            ["Mumbai", "Bangalore", "Delhi", "Pune", "Chennai", "Remote"],
            default=[],
        )
    ...
    with filter_col3:
        skills_filter = st.multiselect(
            "Technical Skills",
            ["Python", "JavaScript", "Java", "React", "AWS", "Docker", "SQL"],
            default=[],
        )
    ...
    if not search_clicked:
        st.info(
            "👆 Enter search criteria and click 'Search Candidates' to find candidates"
        )
    ...
    if search_clicked:
        ...
        has_criteria = (
            ...
            or seniority_filter
            or education_filter
            or experience_filter != "Any"
        )

        if not has_criteria:
            st.warning(
                "⚠️ Please enter search criteria (name, skills, location, etc.) to search for candidates."
            )
            st.info(
                "💡 Try searching by name, selecting skills, or choosing location filters."
            )
        else:
            with st.spinner("🔄 Searching candidates with real API..."):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        st.success(
                            f"✅ Found {count} candidates matching your criteria"
                        )

                        if candidates:
                            for candidate in candidates:
                                with st.expander(
                                    f"👥 {candidate['name']} - Experience: {candidate['experience_years']} years"
                                ):
                                    ...
                                    with col2:
                                        ...
                                        st.write(
                                            f"**Seniority:** {candidate['seniority_level']}"
                                        )
