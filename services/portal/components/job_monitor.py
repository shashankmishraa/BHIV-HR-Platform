            ...
            if jobs:
                ...
                with col3:
                    recent_jobs = sum(
                        1
                        for job in jobs
                        if job.get("created_at", "").startswith("2025")
                    )
                ...
                for client_id, job_list in sorted(client_jobs.items()):
                    ...
                    for job in job_list:
                        with st.expander(
                            f"💼 {job.get('title', 'Untitled Job')} - {job.get('department', 'N/A')}"
                        ):
                            ...
                            with col2:
                                st.write(
                                    f"**Type:** {job.get('employment_type', 'N/A')}"
                                )
                                st.write(f"**Status:** {job.get('status', 'active')}")
                                st.write(
                                    f"**Posted:** {job.get('created_at', 'Unknown')}"
                                )
                            ...
                            with btn_col1:
                                if st.button(
                                    f"🎯 Get AI Matches", key=f"match_{job.get('id')}"
                                ):
                                    st.info(
                                        f"Getting AI matches for Job {job.get('id')}..."
                                    )
                            with btn_col2:
                                if st.button(
                                    f"👥 View Candidates",
                                    key=f"candidates_{job.get('id')}",
                                ):
                                    st.info(
                                        f"Viewing candidates for Job {job.get('id')}..."
                                    )
                            with btn_col3:
                                if st.button(
                                    f"📊 Analytics", key=f"analytics_{job.get('id')}"
                                ):
                                    st.info(
                                        f"Loading analytics for Job {job.get('id')}..."
                                    )
            else:
