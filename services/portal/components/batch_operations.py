...
def show_batch_operations(API_BASE, headers, SECURITY_ENABLED, sanitizer=None):
    ...
    tab1, tab2, tab3 = st.tabs(["📤 Bulk Upload", "🔄 Bulk Processing", "📊 Batch Reports"])
    ...
    with tab2:
        ...
        with col2:
            ...
            if st.button("📊 Generate All Assessment Reports", use_container_width=True):
    ...
    with tab3:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                with col2:
                    processed = len([c for c in candidates if c.get('status') != 'pending'])
                    st.metric("Processed", processed)
                with col3:
                    pending = len([c for c in candidates if c.get('status') == 'pending'])
                    st.metric("Pending", pending)
                with col4:
                    success_rate = (processed / len(candidates) * 100) if candidates else 0
