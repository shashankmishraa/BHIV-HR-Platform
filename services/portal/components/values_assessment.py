    ...
    with st.form("feedback_form"):
        ...
        with col1:
            candidate_name = st.text_input(
                "Candidate Name", placeholder="Full name of the candidate"
            )
            candidate_id = st.number_input("Candidate ID", min_value=1, step=1)
            job_title = st.text_input(
                "Applied Position", placeholder="Position they applied for"
            )
        ...
        overall_recommendation = st.selectbox(
        ...
        submitted = st.form_submit_button(
            "📤 Submit Assessment", use_container_width=True
        )
