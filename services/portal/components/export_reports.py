...
def show_export_reports(API_BASE, headers):
    ...
    st.write(
        "Comprehensive assessment reports with all feedback, interviews, and shortlist data"
    )
    ...
    try:
        ...
        interviews_response = httpx.get(
            f"{API_BASE}/v1/interviews", headers=headers, timeout=10.0
        )
    ...
    with export_col1:
        ...
        if st.button(
            "📥 Export All Candidates with Assessments", use_container_width=True
        ):
            try:
                ...
                if candidates:
                    output = io.StringIO()
                    output.write(
                        "name,email,phone,location,skills,experience,education,interview_status,interviewer,interview_date,feedback_submitted,values_integrity,values_honesty,values_discipline,values_hardwork,values_gratitude,overall_recommendation,shortlist_status\n"
                    )
                    ...
                    for candidate in candidates:
                        ...
                        # Find interview data
                        candidate_interview = next(
                            (
                                i
                                for i in interviews
                                if str(i.get("candidate_id"))
                        ...
                        if candidate_interview:
                            ...
                            interviewer = str(
                                candidate_interview.get("interviewer", "Not Assigned")
                            ).replace(",", ";")
                            interview_date = str(
                                candidate_interview.get(
                                    "interview_date", "Not Scheduled"
                                )
                            ).replace(",", ";")
                            feedback_submitted = (
                                "Yes" if candidate_interview.get("notes") else "Pending"
                            )
                        else:
                        ...
                        values_integrity = candidate.get(
                            "values_integrity", "Not Assessed"
                        )
                        values_honesty = candidate.get("values_honesty", "Not Assessed")
                        values_discipline = candidate.get(
                            "values_discipline", "Not Assessed"
                        )
                        values_hardwork = candidate.get(
                            "values_hardwork", "Not Assessed"
                        )
                        values_gratitude = candidate.get(
                            "values_gratitude", "Not Assessed"
                        )
                        overall_recommendation = candidate.get(
                            "overall_recommendation", "Pending Review"
                        )
                        shortlist_status = (
                            "Yes"
                            if candidate.get("status", "").lower()
                            in ["shortlisted", "interviewed", "offered"]
                            else "Under Review"
                        )

                        output.write(
                            f"{name},{email},{phone},{location},{skills},{experience},{education},{interview_status},{interviewer},{interview_date},{feedback_submitted},{values_integrity},{values_honesty},{values_discipline},{values_hardwork},{values_gratitude},{overall_recommendation},{shortlist_status}\n"
                        )
                    ...
                    st.success(
                        f"✅ Complete assessment report ready ({len(candidates)} candidates with all data)"
                    )
                else:
    ...
    with export_col3:
        ...
        job_id_shortlist = st.number_input(
            "Job ID for Shortlist Export",
            min_value=1,
            value=1,
            key="shortlist_export_job_id",
        )
    ...
    with action_col1:
        if st.button("📊 Generate Assessment Summary", use_container_width=True):
            ...
            st.success(
                "✅ Assessment summary generated! Use export buttons above to download."
            )
    ...
    with action_col2:
        if st.button("🏆 Update Values Scores", use_container_width=True):
            ...
            st.success(
                "✅ Values scores updated! Latest assessments are now available."
            )
