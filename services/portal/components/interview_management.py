    ...
    with tab1:
        ...
        with st.form("interview_form"):
            ...
            submitted = st.form_submit_button(
                "📅 Schedule Interview", use_container_width=True
            )
            ...
            if response.status_code == 200:
                ...
                if interviews:
                    for interview in interviews:
                        ...
                        with st.expander(
                            f"📅 {interview.get('candidate_name', 'Unknown')} - {interview_date}"
                        ):
                            col1, col2 = st.columns(2)
                            with col1:
                                st.write(
                                    f"**Candidate:** {interview.get('candidate_name', 'Unknown')}"
                                )
                                st.write(f"**Date:** {interview_date}")
                                st.write(
                                    f"**Job:** {interview.get('job_title', 'Unknown')}"
                                )
                            with col2:
                                st.write(
                                    f"**Interviewer:** {interview.get('interviewer', 'Unknown')}"
                                )
                                st.write(
                                    f"**Status:** {interview.get('status', 'Unknown')}"
                                )
