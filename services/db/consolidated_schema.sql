...
-- Complete unified schema based on all service requirements
-- Version: 4.0.0 - Consolidated Production Schema
...
-- ============================================================================
-- MIGRATION SUPPORT (Add missing columns to existing tables)
-- ============================================================================

DO $$ 
BEGIN
    -- Add missing columns to candidates table if they don't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='candidates' AND column_name='average_score') THEN
        ALTER TABLE candidates ADD COLUMN average_score DECIMAL(3,2) DEFAULT 0.0;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='candidates' AND column_name='status') THEN
        ALTER TABLE candidates ADD COLUMN status VARCHAR(50) DEFAULT 'applied';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='candidates' AND column_name='updated_at') THEN
        ALTER TABLE candidates ADD COLUMN updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP;
    END IF;
    
    -- Add missing columns to feedback table
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='feedback' AND column_name='average_score') THEN
        ALTER TABLE feedback ADD COLUMN average_score DECIMAL(3,2);
    END IF;
    
    -- Add missing columns to interviews table
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='interviews' AND column_name='interview_type') THEN
        ALTER TABLE interviews ADD COLUMN interview_type VARCHAR(100) DEFAULT 'Technical';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='jobs' AND column_name='client_id') THEN
        ALTER TABLE jobs ADD COLUMN client_id VARCHAR(100);
    END IF;
END $$;
...
-- Candidate summary view with latest feedback
...
GROUP BY c.id, c.name, c.email, c.phone, c.location, c.experience_years, 
         c.technical_skills, c.seniority_level, c.education_level, c.status, 
         c.average_score, c.created_at;
...
SELECT 
    ...
    j.status,
    j.created_at,
    COUNT(DISTINCT mc.candidate_id) as matched_candidates,
    COUNT(DISTINCT i.candidate_id) as interviewed_candidates,
    COUNT(DISTINCT o.candidate_id) as offered_candidates,
...
GROUP BY j.id, j.title, j.department, j.location, j.experience_level, 
         j.client_id, j.status, j.created_at;
...
DO $$
DECLARE
    required_tables TEXT[] := ARRAY['candidates', 'jobs', 'feedback', 'interviews', 'offers', 'users', 'clients', 'matching_cache', 'audit_logs', 'rate_limits'];
    table_name TEXT;
...
BEGIN
    ...
    ELSE
        RAISE NOTICE 'All required tables exist. Schema validation successful.';
    END IF;
END $$;
...
('4.0.0', 'Consolidated schema with all service requirements')
ON CONFLICT (version) DO UPDATE SET applied_at = CURRENT_TIMESTAMP;

-- Final success message
SELECT 'BHIV HR Platform Consolidated Schema v4.0.0 - Successfully Applied' as statusdidates USING gin(to_tsvector('english', technical_skills));
CREATE INDEX IF NOT EXISTS idx_candidates_created_at ON candidates(created_at);
...
-- Final success message
SELECT 'BHIV HR Platform Consolidated Schema v4.0.0 - Successfully Applied' as status;