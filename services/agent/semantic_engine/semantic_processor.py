...
class SemanticProcessor:
    ...
    def process_candidate_profile(
        ...
        processed["skill_categories"] = self._categorize_skills(
            processed["extracted_skills"]
        )
    ...
    def process_job_requirements(self, job_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        processed["experience_requirements"] = self._extract_experience_requirements(
            full_text
        )
    ...
    def semantic_match(
        ...
        match_result = self.advanced_matcher.advanced_match(
    ...
    def _parse_education(self, education: str) -> Dict[str, Any]:
        ...
        if any(term in education_lower for term in ["phd", "doctorate", "ph.d"]):
            ...
            term in education_lower for term in ["bachelor", "b.tech", "be", "bs"]
        ):
    ...
    def _normalize_location(self, location: str) -> Dict[str, Any]:
        ...
        remote_friendly = any(
            term in location_lower for term in ["remote", "anywhere", "wfh"]
        )
    ...
    def _calculate_completeness(self, candidate_data: Dict[str, Any]) -> float:
        ...
        required_score = sum(
            1 for field in required_fields if candidate_data.get(field)
        )
        optional_score = sum(
            1 for field in optional_fields if candidate_data.get(field)
        )
    ...
    def _extract_location_requirements(
        self, job_location: str, job_text: str
    ) -> Dict[str, Any]:
        ...
        remote_options = {
            "fully_remote": any(
                term in text_lower
                for term in ["fully remote", "100% remote", "remote only"]
            ),
            "hybrid": any(
                term in text_lower for term in ["hybrid", "flexible", "part remote"]
            ),
            "on_site": any(
                term in text_lower for term in ["on-site", "onsite", "office based"]
            ),
        }
    ...
    def _calculate_semantic_enhancements(
        self, processed_job: Dict, processed_candidate: Dict
    ) -> Dict[str, Any]:
        ...
        candidate_categories = set(
            processed_candidate.get("skill_categories", {}).keys()
        )
        ...
        return {
            ...
            "semantic_overall_enhancement": round(
                (category_diversity + profile_quality + exp_alignment) / 3, 3
            ),
        }

    def _calculate_experience_alignment(
        self, job_req: Dict, candidate_exp: Dict
    ) -> float:
