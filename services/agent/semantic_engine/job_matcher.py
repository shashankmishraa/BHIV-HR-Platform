...
class SemanticJobMatcher:
    ...
    def calculate_skill_similarity(
        self, job_skills: List[str], candidate_skills: List[str]
    ) -> float:
            ...
            if job_skill in self.skill_embeddings:
                ...
                for cand_skill in candidate_skills:
                    if cand_skill in self.skill_embeddings:
                        ...
                        similarity = np.dot(job_vec, cand_vec) / (
                            np.linalg.norm(job_vec) * np.linalg.norm(cand_vec)
                        )
    ...
    def match(
        ...
        # Domain alignment
        domain_score = self._calculate_domain_alignment(
    ...
    def _calculate_experience_match(
        self, job_level: str, candidate_experience: int
    ) -> float:
    ...
    def _calculate_domain_alignment(
        self, job_requirements: str, candidate_skills: str
    ) -> float:
