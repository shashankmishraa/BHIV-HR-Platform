    ...
    for var_name, expected_value in required_vars.items():
        ...
        if var_name == "API_KEY_SECRET":
            if any(demo_key in current_value.lower() for demo_key in demo_keys):
                errors.append(f"❌ {var_name} contains demo/test value: {current_value[:8]}...")
                continue
            ...
            if current_value == expected_value:
                ...
                warnings.append(f"⚠️  {var_name}: Using custom production key (not default)")
        ...
        # Special validation for JWT_SECRET
            ...
            if current_value == expected_value:
                ...
                warnings.append(f"⚠️  {var_name}: Using custom production secret (not default)")
        
        # Environment validation
        elif var_name == "ENVIRONMENT":
            if current_value.lower() != "production":
                errors.append(f"❌ {var_name} must be 'production', got: {current_value}")
            else:
