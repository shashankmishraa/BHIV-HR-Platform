...
class EndpointAnalyzer:
    ...
    async def test_endpoint(self, client: httpx.AsyncClient, method: str, url: str, data: Dict = None) -> Dict:
    ...
    async def test_gateway_endpoints(self, client: httpx.AsyncClient):
        ...
        endpoints = [
            ...
            ("POST", f"{self.gateway_url}/v1/jobs", {
                "title": "Test Job",
            ...
            ("POST", f"{self.gateway_url}/v1/candidates", {
                "name": "Test Candidate",
            ...
            # Auth endpoints
            ("POST", f"{self.gateway_url}/v1/auth/login", {
                "username": "test",
                "password": "test123"
            }),
            ...
            ("POST", f"{self.gateway_url}/v1/workflows/trigger", {
    ...
    async def test_agent_endpoints(self, client: httpx.AsyncClient):
        ...
        endpoints = [
            ...
            # Matching endpoints
            ("POST", f"{self.agent_url}/v1/match/candidates", {
                "job_id": 1,
                "requirements": ["Python", "FastAPI"],
                "limit": 5
            }),
            ("POST", f"{self.agent_url}/v1/match/score", {
                "candidate_id": 1,
                "job_id": 1
            }),
            ("POST", f"{self.agent_url}/v1/match/bulk", {
                "job_ids": [1, 2],
                "candidate_ids": [1, 2, 3]
            }),
    ...
    async def run_analysis(self):
        ...
        # Generate summary
        total_endpoints = len(self.results["functional"]) + len(self.results["non_functional"])
        ...
        self.results["summary"] = {
            ...
            "success_rate": (functional_count / total_endpoints * 100) if total_endpoints > 0 else 0,
    ...
    def print_results(self):
        ...
        if self.results["functional"]:
            ...
            for endpoint in self.results["functional"][:10]:  # Show first 10
                print(f"  {endpoint['method']} {endpoint['url']} - {endpoint['status_code']} ({endpoint.get('response_time', 0):.2f}s)")
