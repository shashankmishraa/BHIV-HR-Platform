...
def comprehensive_system_assessment():
    ...
    for method, url, name in core_tests:
        ...
        log(
            f"  {status} {name}: {result['status_code']} ({result.get('response_time', 0):.1f}ms)"
        )
    ...
    log(
        f"Core API Functionality: {core_success}/4 - {'EXCELLENT' if core_success >= 4 else 'NEEDS ATTENTION'}"
    )

    # Job management
    job_success = sum(1 for r in results[4:6] if r["success"])
    log(
        f"Job Management: {job_success}/2 - {'EXCELLENT' if job_success >= 2 else 'NEEDS ATTENTION'}"
    )

    # Candidate management
    candidate_success = sum(1 for r in results[6:8] if r["success"])
    log(
        f"Candidate Management: {candidate_success}/2 - {'EXCELLENT' if candidate_success >= 2 else 'NEEDS ATTENTION'}"
    )

    # AI matching
    ai_success = sum(1 for r in results[8:11] if r["success"])
    log(
        f"AI Matching Engine: {ai_success}/3 - {'EXCELLENT' if ai_success >= 3 else 'NEEDS ATTENTION'}"
    )

    # Security
    security_success = sum(1 for r in results[11:15] if r["success"])
    log(
        f"Security Features: {security_success}/4 - {'EXCELLENT' if security_success >= 4 else 'NEEDS ATTENTION'}"
    )
