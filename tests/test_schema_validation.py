...
def test_candidate_bulk_upload_with_correct_schema():
    ...
    try:
        response = requests.post(f"{BASE_URL}/v1/candidates/bulk", headers=HEADERS, json=bulk_data, timeout=10)
...
def test_interview_scheduling_with_correct_schema():
    ...
    try:
        response = requests.post(f"{BASE_URL}/v1/interviews", headers=HEADERS, json=interview_data, timeout=10)
...
def test_security_endpoints_with_correct_data():
    ...
    try:
        response = requests.post(f"{BASE_URL}/v1/security/test-xss", headers=HEADERS, json=xss_data, timeout=10)
    ...
    try:
        response = requests.post(f"{BASE_URL}/v1/security/test-sql-injection", headers=HEADERS, json=sql_data, timeout=10)
...
def test_password_management_endpoints():
    ...
    try:
        response = requests.post(f"{BASE_URL}/v1/password/validate", headers=HEADERS, json=password_data, timeout=10)
        log(f"Password Validation Response: {response.status_code}")
        if response.status_code == 200:
            log("✅ Password validation successful")
            result = response.json()
            log(f"Password strength: {result.get('password_strength')}, Score: {result.get('score')}")
        else:
    ...
    try:
        response = requests.post(f"{BASE_URL}/v1/password/reset", headers=HEADERS, json=email_data, timeout=10)
...
def test_csp_management_endpoints():
    ...
    try:
        response = requests.post(f"{BASE_URL}/v1/csp/report", headers=HEADERS, json=csp_report_data, timeout=10)
    ...
    csp_policy_data = {"policy": "default-src 'self'; script-src 'self' 'unsafe-inline'"}
    try:
        response = requests.put(f"{BASE_URL}/v1/csp/policy", headers=HEADERS, json=csp_policy_data, timeout=10)
...
def test_session_management_endpoints():
    ...
    try:
        response = requests.post(f"{BASE_URL}/v1/sessions/create", headers=HEADERS, json=login_data, timeout=10)
    ...
    try:
        response = requests.post(f"{BASE_URL}/v1/sessions/logout", headers=HEADERS, json=logout_data, timeout=10)
...
def test_client_portal_login():
    ...
    try:
        response = requests.post(f"{BASE_URL}/v1/client/login", headers=HEADERS, json=client_login_data, timeout=10)
...
def test_monitoring_endpoints_with_parameters():
    ...
    try:
        response = requests.get(f"{BASE_URL}/monitoring/logs/search?query=error&hours=24", headers=HEADERS, timeout=10)
...
def test_ai_matching_with_different_parameters():
    ...
    for job_id, limit in test_cases:
        try:
            response = requests.get(f"{BASE_URL}/v1/match/{job_id}/top?limit={limit}", headers=HEADERS, timeout=15)
            log(f"AI Matching (Job {job_id}, Limit {limit}) Response: {response.status_code}")
            if response.status_code == 200:
                result = response.json()
                log(f"✅ AI matching successful - {len(result.get('matches', []))} matches found")
...
def test_candidate_search_with_filters():
    ...
    for params in search_params:
        try:
            response = requests.get(f"{BASE_URL}/v1/candidates/search", headers=HEADERS, params=params, timeout=10)
