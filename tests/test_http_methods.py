...
def test_unsupported_methods():
    ...
    endpoints = [("API Gateway Root", f"{API_BASE}/"), ("AI Agent Root", f"{AI_BASE}/")]
        ...
        try:
            ...
            if get_response.status_code == head_response.status_code:
                ...
                if len(head_response.content) == 0:
                    ...
                    print(
                        f"    ! HEAD response has body: {len(head_response.content)} bytes"
                    )

            else:
                results[name] = (
                    f"INCONSISTENT_STATUS_GET_{get_response.status_code}_HEAD_{head_response.status_code}"
                )
                print(
                    f"  {name}: INCONSISTENT STATUS - GET: {get_response.status_code}, HEAD: {head_response.status_code}"
                )
...
def test_cors_preflight():
    ...
    endpoints = [("API Gateway", f"{API_BASE}/"), ("AI Agent", f"{AI_BASE}/")]
    ...
    for name, url in endpoints:
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"  {name}: CORS PREFLIGHT ERROR (Status: {response.status_code})"
                )
