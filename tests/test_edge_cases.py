    ...
    try:
        ...
        if response.status_code == 400:
            ...
            if "Candidates list cannot be empty" in result.get("detail", ""):
                ...
                print(f"  Empty candidates list: FAILED - Wrong error message: {result.get('detail')}")
                return False
        else:
            print(f"  Empty candidates list: FAILED - Expected 400, got {response.status_code}")
            return False
...
def test_interview_field_names():
    ...
    try:
        ...
        if response.status_code == 200:
            result = response.json()
            print(f"  Interview creation: PASSED - Created interview ID {result.get('interview_id')}")
            return True
        else:
