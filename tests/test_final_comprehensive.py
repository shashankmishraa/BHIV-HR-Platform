...
class ComprehensiveTester:
                    ...
                    try:
                        ...
                        if isinstance(json_data, dict):
                            if "matches" in json_data:
                                ...
                                self.log(
                                    f"  Candidates: {len(json_data['candidates'])}"
                                )
    ...
    def test_candidate_management(self):
        ...
        self.test_endpoint(
            "GET", f"{BASE_URL}/v1/candidates/job/1", 200, "Candidates by Job"
        )

        # Test candidate search with parameters
        self.test_endpoint(
            "GET",
            f"{BASE_URL}/v1/candidates/search?skills=python",
            200,
            "Search Candidates",
        )
        ...
        self.test_endpoint(
            "POST", f"{BASE_URL}/v1/candidates/bulk", bulk_data, 200, "Bulk Upload"
        )
    ...
    def test_ai_matching(self):
        ...
        self.test_endpoint(
            "GET", f"{BASE_URL}/v1/match/1/top", 200, "AI Matching - Job 1"
        )
        self.test_endpoint(
            "GET", f"{BASE_URL}/v1/match/1/top?limit=5", 200, "AI Matching - Limited"
        )
        self.test_endpoint(
            "GET", f"{BASE_URL}/v1/match/performance-test", 200, "Performance Test"
        )
        self.test_endpoint(
    ...
    def test_password_management(self):
        ...
        self.test_endpoint(
            "GET", f"{BASE_URL}/v1/password/strength-test", 200, "Strength Test"
        )
        self.test_endpoint(
        ...
        self.test_endpoint(
            "POST", f"{BASE_URL}/v1/password/reset", email_data, 200, "Password Reset"
        )
    ...
    def test_csp_management(self):
        ...
        self.test_endpoint(
            "POST", f"{BASE_URL}/v1/csp/report", csp_report, 200, "CSP Report"
        )

        policy_data = {"policy": "default-src 'self'"}
        self.test_endpoint(
            "PUT", f"{BASE_URL}/v1/csp/policy", policy_data, 200, "Update CSP Policy"
        )
    ...
    def test_session_management(self):
        ...
        self.test_endpoint(
            "POST", f"{BASE_URL}/v1/sessions/create", login_data, 200, "Create Session"
        )
        ...
        self.test_endpoint(
            "POST", f"{BASE_URL}/v1/sessions/logout", logout_data, 200, "Logout Session"
        )
    ...
    def test_client_portal(self):
        ...
        self.test_endpoint(
            "POST", f"{BASE_URL}/v1/client/login", client_data, 200, "Client Login"
        )
    ...
    def generate_final_report(self):
        ...
        self.log(f"\nKEY METRICS:")
        self.log(
            f"- Core API Functionality: {'PASS' if success_rate >= 80 else 'NEEDS ATTENTION'}"
        )
        self.log(
            f"- Security Features: {'ROBUST' if success_rate >= 85 else 'NEEDS REVIEW'}"
        )
        self.log(
            f"- AI Matching Engine: {'OPERATIONAL' if success_rate >= 75 else 'NEEDS FIXES'}"
        )
        self.log(
            f"- Database Integration: {'STABLE' if success_rate >= 80 else 'UNSTABLE'}"
        )
        self.log(
            f"- Monitoring System: {'COMPREHENSIVE' if success_rate >= 85 else 'BASIC'}"
        )
