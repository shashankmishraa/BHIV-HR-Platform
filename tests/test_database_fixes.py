...
def test_database_fixes():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            has_status_column = data.get("schema_status", {}).get("candidates_has_status_column", False)
            candidate_count = data.get("tables", {}).get("candidates", {}).get("count", 0)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "successfully" in data.get("message", "").lower() or "up to date" in data.get("message", "").lower():
    ...
    total_tests += 1
    try:
        response = requests.get(f"{base_url}/v1/candidates/search?skills=python", headers=headers)
        if response.status_code == 200:
            data = response.json()
            if "error" not in data:
                print(f"[PASS] Candidate search working - found {data.get('count', 0)} candidates")
                tests_passed += 1
            else:
    ...
    total_tests += 1
    try:
        response = requests.post(f"{base_url}/v1/candidates/sample-data", headers=headers)
        if response.status_code == 200:
            data = response.json()
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if len(matches) > 0:
                ...
                print("[INFO] AI matching working but no matches (expected if no candidates)")
                tests_passed += 1
        else:
