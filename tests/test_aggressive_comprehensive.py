...
class AggressiveTester:
    ...
    def test_endpoint(self, method, url, data=None, expected_status=200, test_name=""):
        ...
        try:
            ...
            if success:
                ...
                self.log(f"✅ {test_name}: {method} {url} - {response.status_code} ({duration*1000:.1f}ms)")
            else:
                self.failed_tests.append(result)
                self.log(f"❌ {test_name}: {method} {url} - Expected {expected_status}, got {response.status_code}", "ERROR")
    ...
    def test_job_management(self):
        ...
        self.test_endpoint("GET", f"{BASE_URL}/v1/jobs", expected_status=200, test_name="List Jobs")
    ...
    def test_candidate_management(self):
        ...
        candidate_tests = [
            ...
            ("POST", f"{BASE_URL}/v1/candidates/search", {"query": "python"}, 200, "Search Candidates"),
        ]
    ...
    def test_ai_matching(self):
        ...
        ai_tests = [
            ...
            ("GET", f"{BASE_URL}/v1/match/performance-test", 200, "AI Performance Test"),
    ...
    def test_security_features(self):
        ...
        security_tests = [
            ...
            ("POST", f"{BASE_URL}/v1/security/test-xss", {"input": "<script>alert('test')</script>"}, 200, "XSS Protection"),
            ("POST", f"{BASE_URL}/v1/security/test-sql-injection", {"query": "'; DROP TABLE users; --"}, 200, "SQL Injection Protection"),
            ...
            ("GET", f"{BASE_URL}/v1/security/rate-limit-status", 200, "Rate Limit Status"),
            ("GET", f"{BASE_URL}/v1/security/cors-config", 200, "CORS Configuration"),
            ("GET", f"{BASE_URL}/v1/security/cookie-config", 200, "Cookie Configuration"),
        ]
    ...
    def test_authentication(self):
        ...
        auth_tests = [
            ("POST", f"{BASE_URL}/v1/auth/2fa/setup", {"user_id": "test_user"}, 200, "2FA Setup"),
            ("POST", f"{BASE_URL}/v1/auth/2fa/verify", {"user_id": "test_user", "token": "123456"}, 200, "2FA Verify"),
            ("POST", f"{BASE_URL}/v1/auth/2fa/login", {"username": "test", "password": "test", "token": "123456"}, 200, "2FA Login"),
            ("GET", f"{BASE_URL}/v1/auth/api-keys", 200, "List API Keys"),
            ("POST", f"{BASE_URL}/v1/auth/api-keys", {"name": "test_key"}, 200, "Create API Key"),
        ]
    ...
    def test_password_management(self):
        ...
        password_tests = [
            ("POST", f"{BASE_URL}/v1/password/validate", {"password": "TestPass123!"}, 200, "Password Validation"),
            ("GET", f"{BASE_URL}/v1/password/generate", 200, "Generate Password"),
            ("GET", f"{BASE_URL}/v1/password/policy", 200, "Password Policy"),
            ("POST", f"{BASE_URL}/v1/password/change", {"old_password": "old", "new_password": "new"}, 200, "Change Password"),
            ...
            ("POST", f"{BASE_URL}/v1/password/reset", {"email": "test@example.com"}, 200, "Password Reset"),
        ]
    ...
    def test_csp_management(self):
        ...
        csp_tests = [
            ...
            ("POST", f"{BASE_URL}/v1/csp/report", {"violation": "test"}, 200, "CSP Report"),
            ("PUT", f"{BASE_URL}/v1/csp/policy", {"policy": "default-src 'self'"}, 200, "Update CSP Policy"),
        ]
    ...
    def test_session_management(self):
        ...
        session_tests = [
            ("POST", f"{BASE_URL}/v1/sessions/create", {"user_id": "test_user"}, 200, "Create Session"),
            ("GET", f"{BASE_URL}/v1/sessions/validate", 200, "Validate Session"),
            ("POST", f"{BASE_URL}/v1/sessions/logout", {"session_id": "test_session"}, 200, "Logout Session"),
        ]
    ...
    def test_interview_management(self):
        ...
        interview_tests = [
            ...
            ("POST", f"{BASE_URL}/v1/interviews", {"candidate_id": 1, "job_id": 1, "scheduled_at": "2025-02-01T10:00:00Z"}, 200, "Schedule Interview"),
        ]
        
        for method, url, expected, name in interview_tests:
            if method == "POST":
                data = {"candidate_id": 1, "job_id": 1, "scheduled_at": "2025-02-01T10:00:00Z"}
                self.test_endpoint(method, url, data, expected, name)
            else:
                self.test_endpoint(method, url, expected_status=expected, test_name=name)
    ...
    def test_database_management(self):
        ...
        db_tests = [
            ...
            ("POST", f"{BASE_URL}/v1/database/add-interviewer-column", {}, 200, "Add Interviewer Column"),
        ]
    ...
    def test_client_portal(self):
        ...
        client_tests = [
            ("POST", f"{BASE_URL}/v1/client/login", {"username": "TECH001", "password": "demo123"}, 200, "Client Login"),
        ]
    ...
    def test_performance_benchmarks(self):
        ...
        for endpoint in performance_endpoints:
            ...
            if times:
                ...
                self.log(f"Performance {endpoint}: Avg={avg_time:.1f}ms, Min={min_time:.1f}ms, Max={max_time:.1f}ms")
    ...
    def generate_final_report(self):
        ...
        success_rate = (len(self.passed_tests) / self.total_tests * 100) if self.total_tests > 0 else 0
        ...
        if self.failed_tests:
            ...
            for test in self.failed_tests:
                self.log(f"  - {test.get('test_name', 'Unknown')}: {test.get('error', 'Status code mismatch')}")
