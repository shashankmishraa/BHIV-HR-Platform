    ...
    try:
        ...
        if app.SEMANTIC_ENABLED:
            ...
            if app.semantic_processor:
                ...
                match_result = app.semantic_processor.semantic_match(
                    test_job, test_candidate
                )
                ...
                if hasattr(app.semantic_processor, "model_manager"):
                    ...
                    print(
                        f"Model Stats: {stats['skill_embeddings_count']} skills, {stats['job_templates_count']} templates"
                    )
        ...
        print(
            f"   Semantic Engine: {'Enabled' if app.SEMANTIC_ENABLED else 'Fallback'}"
        )
...
def test_semantic_engine_performance():
    ...
    try:
        ...
        start_time = time.time()
        batch_results = app.batch_matcher.batch_match_single_job(
            test_job, test_candidates[:5]
        )
        ...
        assert (
            single_match_time < 1.0
        ), f"Single match too slow: {single_match_time:.3f}s"
