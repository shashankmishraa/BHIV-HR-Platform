        ...
        for i in range(3):
            ...
            try:
                response = requests.get(
                    f"{api_base}{endpoint}", headers=headers, timeout=30
                )
                end_time = time.time()
                ...
                if endpoint == "/v1/match/1/top" and response.status_code == 200:
                    ...
                    print(
                        f"    Processing: {processing_time}, Cache: {cache_hit}, Precomputed: {precomputed}"
                    )
    ...
    for i in range(5):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"  Call {i+1}: {response_time:.3f}s - Cache: {cache_hit}, Precomputed: {precomputed}, Processing: {processing_time}"
                )
            else:
                print(
                    f"  Call {i+1}: {response_time:.3f}s - Status: {response.status_code}"
                )
