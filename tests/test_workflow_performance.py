...
class WorkflowPerformanceTester:
    ...
    def test_job_creation_performance(self, iterations: int = 5) -> Dict[str, Any]:
        ...
        print(
            f"   Benchmark: {'✅ PASSED' if result['benchmark_met'] else '❌ FAILED'} ({self.benchmarks['job_creation']}s)"
        )

        return result

    def test_candidate_upload_performance(
        self, batch_sizes: List[int] = [1, 5, 10, 20]
    ) -> Dict[str, Any]:
        ...
        for batch_size in batch_sizes:
            ...
            try:
                ...
                print(
                    f"      Time: {execution_time:.3f}s | Throughput: {throughput:.1f} candidates/s"
                )
    ...
    def test_ai_matching_performance(
        self, job_id: int = 1, iterations: int = 3
    ) -> Dict[str, Any]:
        ...
        print(f"   Success Rate: {result['success_rate']:.1%}")
        print(
            f"   Benchmark: {'✅ PASSED' if result['benchmark_met'] else '❌ FAILED'} ({self.benchmarks['ai_matching']}s)"
        )

        return result

    def test_concurrent_request_performance(
        self, concurrent_users: int = 5, requests_per_user: int = 3
    ) -> Dict[str, Any]:
        ...
        def make_request(user_id: int, request_id: int) -> Dict[str, Any]:
            ...
            try:
                response = requests.get(
                    f"{self.api_base}/health", headers=self.headers, timeout=10
                )
        ...
        result = {
            ...
            "throughput": throughput,
            "benchmark_met": avg_response_time
            <= self.benchmarks["concurrent_requests"],
        }

        print(f"   Total Time: {total_time:.3f}s | Throughput: {throughput:.1f} req/s")
        print(
            f"   Avg Response: {avg_response_time:.3f}s | Success Rate: {result['success_rate']:.1%}"
        )
        print(
            f"   Benchmark: {'✅ PASSED' if result['benchmark_met'] else '❌ FAILED'} ({self.benchmarks['concurrent_requests']}s)"
        )
    ...
    def test_end_to_end_workflow_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                ...
                "success": True,
                "benchmark_met": total_workflow_time
                <= self.benchmarks["end_to_end_workflow"],
            }
            ...
            print(
                f"   Benchmark: {'✅ PASSED' if result['benchmark_met'] else '❌ FAILED'} ({self.benchmarks['end_to_end_workflow']}s)"
            )
    ...
    def print_performance_summary(self, results: Dict[str, Any]):
        ...
        for test_name, result in results.items():
            if isinstance(result, dict) and "benchmark_met" in result:
                benchmark_status = (
                    "✅ PASSED" if result["benchmark_met"] else "❌ FAILED"
                )

                if test_name == "job_creation":
                    print(
                        f"   Job Creation: {result['avg_time']:.3f}s avg - {benchmark_status}"
                    )
                elif test_name == "ai_matching":
                    print(
                        f"   AI Matching: {result['avg_time']:.3f}s avg - {benchmark_status}"
                    )
                elif test_name == "concurrent_requests":
                    print(
                        f"   Concurrent Requests: {result['avg_response_time']:.3f}s avg - {benchmark_status}"
                    )
                elif test_name == "end_to_end_workflow":
                    print(
                        f"   End-to-End Workflow: {result['total_time']:.3f}s - {benchmark_status}"
                    )
