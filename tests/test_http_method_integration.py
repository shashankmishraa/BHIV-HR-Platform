...
class HTTPMethodTester:
    ...
    def test_endpoint_methods(
        self, name: str, url: str, methods: List[str], requires_auth: bool = False
    ) -> Dict[str, str]:
        ...
        for method in methods:
            try:
                ...
                if method in [
                    ...
                    "PUT",
                    "DELETE",
                ] and response.status_code in [200, 201, 404, 422]:
                    ...
                    if len(response.content) == 0:
                        ...
                        endpoint_results[method] = (
                            f"SUCCESS_WITH_BODY_{len(response.content)}"
                        )
                    ...
                    method not in ["GET", "POST", "PUT", "DELETE", "HEAD", "OPTIONS"]
    ...
    def test_core_endpoints(self) -> Dict[str, Dict[str, str]]:
        ...
        core_endpoints = [
            ...
            (
                "API Gateway Test Candidates",
                f"{API_BASE}/test-candidates",
                ["GET", "HEAD"],
                True,
            ),
            (
                "API Gateway HTTP Methods Test",
                f"{API_BASE}/http-methods-test",
                ["GET", "HEAD", "OPTIONS"],
                True,
            ),
            (
            ...
            (
                "AI Agent HTTP Methods Test",
                f"{AI_BASE}/http-methods-test",
                ["GET", "HEAD", "OPTIONS"],
                False,
            ),
        ]
        ...
        for name, url, methods, requires_auth in core_endpoints:
            ...
            results[name] = self.test_endpoint_methods(
                name, url, methods, requires_auth
            )
    ...
    def test_api_endpoints(self) -> Dict[str, Dict[str, str]]:
        ...
        api_endpoints = [
            ...
            (
                "Candidates Search",
                f"{API_BASE}/v1/candidates/search",
                ["GET", "HEAD", "POST"],
                True,
            ),
            (
                "Match Candidates",
                f"{API_BASE}/v1/match/1/top",
                ["GET", "HEAD", "POST"],
                True,
            ),
            (
                "Candidate Stats",
                f"{API_BASE}/candidates/stats",
                ["GET", "HEAD", "PUT"],
                True,
            ),
        ]
    ...
    def test_security_endpoints(self) -> Dict[str, Dict[str, str]]:
        ...
        security_endpoints = [
            (
                "Rate Limit Status",
                f"{API_BASE}/v1/security/rate-limit-status",
                ["GET", "HEAD", "POST"],
                True,
            ),
            (
                "Security Headers Test",
                f"{API_BASE}/v1/security/security-headers-test",
                ["GET", "HEAD"],
                True,
            ),
            ...
            (
                "Password Validation",
                f"{API_BASE}/v1/password/validate",
                ["POST", "GET", "HEAD"],
                True,
            ),
        ]
    ...
    def test_concurrent_requests(self) -> Dict[str, str]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            future_to_request = {
                executor.submit(make_request, req): req for req in test_requests
            }
    ...
    def test_response_consistency(self) -> Dict[str, str]:
        ...
        for name, url in test_urls:
            try:
                ...
                content_type_match = get_response.headers.get(
                    "content-type", ""
                ...
                if status_match and content_type_match and head_no_body:
                    ...
                    if not status_match:
                        issues.append(
                            f"STATUS_MISMATCH_{get_response.status_code}_{head_response.status_code}"
                        )
                ...
                for endpoint_name, method_results in category_results.items():
                    ...
                    for method, result in method_results.items():
                        status = (
                            ...
                            if any(
                                success in result
                                for success in ["SUCCESS", "CORRECTLY_REJECTED"]
                            )
                            else "FAILED"
                        )
