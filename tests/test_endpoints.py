...
def test_authentication():
    ...
    try:
        response = requests.get(
            f"{API_BASE}/candidates/stats", headers=HEADERS, timeout=5
        )
        if response.status_code == 200:
...
def test_job_endpoints():
    ...
    try:
        ...
        response = requests.post(
            f"{API_BASE}/v1/jobs", headers=HEADERS, json=job_data, timeout=10
        )
        if response.status_code == 200:
...
def test_candidate_endpoints():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"  POST /v1/candidates/bulk: PASSED - Uploaded {result.get('count', 0)} candidates"
            )

            # GET /v1/candidates/job/1
            response = requests.get(
                f"{API_BASE}/v1/candidates/job/1", headers=HEADERS, timeout=10
            )
            if response.status_code == 200:
                candidates = response.json()
                print(
                    f"  GET /v1/candidates/job/1: PASSED - Found {candidates.get('count', 0)} candidates"
                )

                # GET /v1/candidates/search
                response = requests.get(
                    f"{API_BASE}/v1/candidates/search?job_id=1&skills=Python",
                    headers=HEADERS,
                    timeout=10,
                )
                if response.status_code == 200:
                    ...
                    print(
                        f"  GET /v1/candidates/search: PASSED - Found {search_result.get('count', 0)} candidates"
                    )
                    return True
                else:
                    print(
                        f"  GET /v1/candidates/search: FAILED - Status {response.status_code}"
                    )
                    return False
            else:
                print(
                    f"  GET /v1/candidates/job/1: FAILED - Status {response.status_code}"
                )
                return False
        else:
...
def test_ai_matching():
    ...
    try:
        response = requests.get(
            f"{API_BASE}/v1/match/1/top", headers=HEADERS, timeout=15
        )
        if response.status_code == 200:
            result = response.json()
            candidates = result.get("top_candidates", [])
            print(
                f"  GET /v1/match/1/top: PASSED - Generated {len(candidates)} top candidates"
            )
            return True
        else:
...
def test_feedback_endpoint():
    ...
    try:
        response = requests.post(
            f"{API_BASE}/v1/feedback", headers=HEADERS, json=feedback_data, timeout=10
        )
        if response.status_code == 200:
            result = response.json()
            print(
                f"  POST /v1/feedback: PASSED - Submitted feedback ID {result.get('feedback_id')}"
            )
            return True
        else:
...
def test_interview_endpoint():
    ...
    try:
        ...
        if response.status_code == 200:
            result = response.json()
            print(
                f"  POST /v1/interviews: PASSED - Scheduled interview ID {result.get('interview_id')}"
            )
            return True
        else:
...
def test_offer_endpoint():
    ...
    offer_data = {"candidate_id": 1, "job_id": 1, "salary": 120000, "status": "sent"}

    try:
        response = requests.post(
            f"{API_BASE}/v1/offers", headers=HEADERS, json=offer_data, timeout=10
        )
        if response.status_code == 200:
            result = response.json()
            print(
                f"  POST /v1/offers: PASSED - Created offer ID {result.get('offer_id')}"
            )
            return True
        else:
...
def test_stats_endpoint():
    ...
    try:
        response = requests.get(
            f"{API_BASE}/candidates/stats", headers=HEADERS, timeout=10
        )
        if response.status_code == 200:
            stats = response.json()
    ...
    try:
        ...
        if head_response.status_code == 200:
            ...
            if options_response.status_code == 200:
                ...
                try:
                    trace_response = requests.request(
                        "TRACE", f"{API_BASE}/", timeout=5
                    )
                    if trace_response.status_code == 405:
                        ...
                        print(
                            f"  TRACE / (unsupported): UNEXPECTED STATUS {trace_response.status_code}"
                        )
                        return False
                except Exception:
