#!/usr/bin/env python3
"""
BHIV HR Platform - Comprehensive Service Connection and Routing Audit
...
class ServiceConnectionAuditor:
    def __init__(self):
        ...
        self.test_credentials = {
            ...
            "client_login": {"username": "TECH001", "password": "demo123"}
        }
    ...
    async def audit_service_health(self, service_name: str, service_config: Dict) -> Dict:
        ...
        print(f"🔍 Auditing {service_config['name']}...")
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as session:
                ...
                if "docs_endpoint" in service_config:
                    try:
                        docs_url = service_config["url"] + service_config["docs_endpoint"]
                        async with session.get(docs_url) as response:
                            service_result["endpoints_verified"].append({
                                "endpoint": service_config["docs_endpoint"],
                                "status": response.status,
                                "response_time": round((time.time() - start_time) * 1000, 2)
                            })
                            
                            if response.status != 200:
                                service_result["issues"].append(f"Documentation not accessible: {response.status}")
                    except Exception as e:
                        service_result["issues"].append(f"Documentation endpoint error: {str(e)}")
    ...
    async def verify_gateway_routing(self) -> Dict:
        ...
        print("🔍 Verifying Gateway Routing...")
    ...
    async def verify_agent_integration(self) -> Dict:
        ...
        print("🔍 Verifying AI Agent Integration...")
    ...
    async def verify_cross_service_communication(self) -> Dict:
        """Verify communication between services"""
        print("🔍 Verifying Cross-Service Communication...")
        
        communication_result = {
            "gateway_to_agent": {"status": "unknown", "details": {}},
            "portal_to_gateway": {"status": "unknown", "details": {}},
            "client_portal_to_gateway": {"status": "unknown", "details": {}},
        ...
        try:
            gateway_url = self.services["gateway"]["url"]
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as session:
                # Check if gateway can reach agent through health check
                health_url = gateway_url + "/health/detailed"
                async with session.get(health_url) as response:
                    if response.status == 200:
                        try:
                            data = await response.json()
                            if "dependencies" in data:
                                for dep_name, dep_status in data["dependencies"].items():
                                    if "agent" in dep_name.lower() or "ai" in dep_name.lower():
                                        communication_result["gateway_to_agent"]["status"] = dep_status.get("status", "unknown")
                                        communication_result["gateway_to_agent"]["details"] = dep_status
                        except:
                            communication_result["communication_issues"].append("Failed to parse gateway health response")
        except Exception as e:
            communication_result["communication_issues"].append(f"Gateway-Agent communication test failed: {str(e)}")
    ...
    def analyze_project_structure(self) -> Dict:
        ...
        # Check routing files
        routing_files = [
            "services/gateway/app/modules/core/router.py",
            "services/gateway/app/modules/candidates/router.py",
            "services/gateway/app/modules/jobs/router.py",
            "services/gateway/app/modules/auth/router.py",
            "services/gateway/app/modules/workflows/router.py",
            "services/gateway/app/modules/monitoring/router.py"
        ]
        
        for file_path in routing_files:
            full_path = project_root / file_path
            if full_path.exists():
                structure_result["routing_files"].append({
                    "file": file_path,
                    "exists": True,
                    "module": file_path.split("/")[-2]
                })
            else:
                structure_result["missing_files"].append(file_path)
                structure_result["structure_issues"].append(f"Missing routing file: {file_path}")
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        for service_name, service_result in self.results["services"].items():
            if service_result.get("status") != "reachable":
                recommendations.append(f"🔧 Fix connectivity issues for {service_result['name']}")
            
            if service_result.get("issues"):
                recommendations.append(f"🔧 Address {len(service_result['issues'])} issues in {service_result['name']}")
        
        if self.results["routing_verification"].get("routing_issues"):
            recommendations.append("🔧 Fix API Gateway routing issues")
        
        if self.results["integration_points"].get("communication_issues"):
            recommendations.append("🔧 Resolve cross-service communication problems")
        
        # Add general recommendations
        recommendations.extend([
            "📊 Implement comprehensive monitoring for all service endpoints",
            "🔒 Ensure all services have proper authentication and authorization",
            "⚡ Optimize response times for services with >1000ms latency",
            "📝 Update documentation for any missing or outdated API endpoints",
    ...
    async def run_comprehensive_audit(self) -> Dict:
        ...
        print("🚀 Starting Comprehensive Service Connection and Routing Audit")
        print("=" * 80)
        ...
        cross_service_comm = await self.verify_cross_service_communication()
        
        self.results["integration_points"] = {
            "agent_integration": agent_integration,
            "cross_service_communication": cross_service_comm
        }
        ...
        all_issues.extend(self.results["integration_points"]["cross_service_communication"].get("communication_issues", []))
    ...
    def print_audit_summary(self):
        ...
        print("\n" + "=" * 80)
        print("📋 COMPREHENSIVE SERVICE CONNECTION AUDIT SUMMARY")
        print("=" * 80)
        ...
        print(f"🏗️  Routing Modules Verified: {summary['routing_modules_verified']}")
        print(f"💡 Recommendations: {summary['recommendations_count']}")
        print(f"🎯 Overall Health: {summary['overall_health'].upper()}")
        
        print("\n📋 SERVICE STATUS:")
        for service_name, service_result in self.results["services"].items():
            status_icon = "✅" if service_result.get("status") == "reachable" else "❌"
        ...
        if self.results["issues_found"]:
            print("\n⚠️  ISSUES FOUND:")
            for i, issue in enumerate(self.results["issues_found"][:10], 1):  # Show first 10 issues
                print(f"  {i}. {issue}")
            if len(self.results["issues_found"]) > 10:
                print(f"  ... and {len(self.results['issues_found']) - 10} more issues")
        
        print("\n💡 TOP RECOMMENDATIONS:")
        for i, rec in enumerate(self.results["recommendations"][:5], 1):  # Show top 5 recommendations
            print(f"  {i}. {rec}")
...
async def main():
    ...
    try:
        ...
        print("\n✅ Comprehensive Service Connection and Routing Audit Completed!")
