...
def analyze_import_issues():
    ...
    stdout, stderr, code = run_command(
        "python -c 'import sys; sys.path.append(\"services/gateway\"); from app.workflow_engine import workflow_engine'"
    )
    if code != 0:
        ...
        stdout2, stderr2, code2 = run_command(
            "python -c 'import sys; sys.path.append(\"services/gateway/app\"); from workflow_engine import workflow_engine'"
        )
        if code2 != 0:
...
def main():
    ...
    print("\n" + "=" * 50)
    print(
        "Diagnostic complete. Check the logs above for import errors or startup issues."
    )
